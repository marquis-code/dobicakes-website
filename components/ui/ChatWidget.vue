<template>
  <div class="fixed bottom-8 right-8 z-[100] flex flex-col items-end gap-4">
    ...
    <div v-if="isOpen" 
         :class="[
           'bg-white shadow-2xl border border-slate-100 flex flex-col overflow-hidden animate-slide-up origin-bottom-right transition-all duration-500',
           isMobile ? 'fixed inset-0 w-full h-full rounded-none z-[110]' : 'w-[400px] h-[650px] rounded-3xl'
         ]">
      
      <!-- Engagement Hub (Selection) -->
      <div v-if="!activeChannel" class="flex-grow flex flex-col p-8 space-y-8">
        <div class="flex justify-end">
          <button @click="isOpen = false" class="p-2 hover:bg-slate-50 rounded-full transition-colors">
            <LucideX :size="24" class="text-slate-400" />
          </button>
        </div>
        <div class="text-center space-y-3">
          <h3 class="text-3xl font-serif italic text-brand-charcoal">How can we help?</h3>
          <p class="text-xs text-slate-400 tracking-widest uppercase font-bold">Select your preferred channel</p>
        </div>

        <div class="space-y-4 pt-4">
          <button @click="activeChannel = 'chat'" class="w-full flex items-center gap-6 p-6 bg-slate-50 rounded-2xl hover:bg-brand-gold hover:text-white transition-all group shadow-sm hover:shadow-xl hover:-translate-y-1">
            <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
              <LucideMessageSquare :size="22" class="text-brand-gold" />
            </div>
            <div class="text-left">
              <p class="text-sm font-bold tracking-tight">Live Concierge</p>
              <p class="text-[10px] opacity-60">Chat with our artisans in real-time</p>
            </div>
          </button>

          <a href="https://wa.me/2349000000000" target="_blank" class="w-full flex items-center gap-6 p-6 bg-slate-50 rounded-2xl hover:bg-[#25D366] hover:text-white transition-all group shadow-sm hover:shadow-xl hover:-translate-y-1">
            <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
              <LucideMessageCircle :size="22" class="text-[#25D366]" />
            </div>
            <div class="text-left">
              <p class="text-sm font-bold tracking-tight">WhatsApp Business</p>
              <p class="text-[10px] opacity-60">Instant artisanal messaging</p>
            </div>
          </a>

          <a href="tel:+2349000000000" class="w-full flex items-center gap-6 p-6 bg-slate-50 rounded-2xl hover:bg-slate-900 hover:text-white transition-all group shadow-sm hover:shadow-xl hover:-translate-y-1">
            <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
              <LucidePhone :size="22" class="text-slate-900" />
            </div>
            <div class="text-left">
              <p class="text-sm font-bold tracking-tight">Direct Call</p>
              <p class="text-[10px] opacity-60">Speak with our master bakers</p>
            </div>
          </a>
        </div>
      </div>
      ...
      <div v-else class="flex flex-col h-full relative">
        ...
        <div class="bg-brand-charcoal p-6 text-white flex justify-between items-center shrink-0">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h4 class="text-sm font-bold tracking-widest uppercase">Live Concierge</h4>
              <p class="text-[9px] text-emerald-400 font-bold tracking-widest uppercase animate-pulse">Artisan Online</p>
            </div>
          </div>
        ...
        <!-- Connection State -->
        <div v-if="!isInitiated" class="flex-grow flex flex-col items-center justify-center p-12 space-y-8 text-center bg-slate-50/50">
          <div class="w-24 h-24 bg-white rounded-3xl shadow-2xl flex items-center justify-center animate-bounce-subtle">
            <LucideHeart :size="48" class="text-brand-gold" />
          </div>
          <div class="space-y-3">
            <h3 class="text-xl font-serif italic text-brand-charcoal">Establish Connection</h3>
            <p class="text-xs text-slate-400 leading-relaxed px-4">Provide your details so our artisans can provide the perfect consultation for your celebration.</p>
          </div>
          <div class="w-full space-y-4">
            <input v-model="userForm.name" type="text" placeholder="Full Name" class="w-full bg-white border border-slate-100 rounded-2xl py-4 px-6 outline-none focus:border-brand-gold text-sm transition-all shadow-sm" />
            <input v-model="userForm.email" type="email" placeholder="Email Address" class="w-full bg-white border border-slate-100 rounded-2xl py-4 px-6 outline-none focus:border-brand-gold text-sm transition-all shadow-sm" />
            <button @click="initiateChat" :disabled="!userForm.name || !userForm.email" class="w-full bg-slate-900 text-white py-5 rounded-2xl text-[11px] font-bold tracking-[0.3em] uppercase hover:bg-brand-gold transition-all disabled:opacity-30 shadow-2xl shadow-slate-900/20 active:scale-95">
              Begin Consultation
            </button>
          </div>
        </div>
          ...
          <div class="flex-grow overflow-y-auto p-6 space-y-6 bg-slate-50/30 custom-scrollbar" ref="messageContainer">
            <div v-for="(msg, i) in messages" :key="i" 
              ...
              <div :class="[
                ...
                <span class="text-[8px] opacity-40 px-5 pb-3 block text-right uppercase font-bold tracking-tighter">
