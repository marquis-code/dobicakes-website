  ...
  <nav class="bg-white/95 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50">
    <div class="section-container !py-0 flex items-center justify-between h-16 md:h-20">
      ...
      <NuxtLink to="/" class="flex items-center gap-2">
        <div class="w-9 h-9 bg-brand-gold rounded-full flex items-center justify-center text-white font-serif font-bold text-lg">
          D
        </div>
      ...
      <!-- Desktop Nav -->
      <div class="hidden lg:flex items-center gap-10 text-[11px] font-bold text-brand-charcoal  tracking-[0.2em]">
      ...
      <div class="flex items-center gap-4">
        ...
        <NuxtLink v-if="!isLoggedIn" to="/auth/login" class="hidden md:block text-[10px] font-bold  tracking-widest hover:text-brand-gold transition-colors">
          Login
        </NuxtLink>
        <NuxtLink v-else to="/profile" class="hidden md:flex items-center gap-2 hover:text-brand-gold transition-colors">
          <LucideUser :size="18" />
          <span class="hidden xl:inline text-[10px] font-bold  tracking-widest">{{ user?.firstName }}</span>
        </NuxtLink>
      ...
      <div v-if="mobileMenuOpen" class="lg:hidden bg-white border-t border-gray-50 shadow-xl">
        <div class="section-container !py-8 space-y-6">
          ...
          <div class="pt-4 space-y-4">
            <NuxtLink v-if="!isLoggedIn" to="/auth/login" @click="mobileMenuOpen = false" class="block w-full py-4 bg-brand-charcoal text-white text-center text-[10px] font-bold  tracking-[0.3em]">
              Login / Register
            </NuxtLink>
            <button v-else @click="logOut(); mobileMenuOpen = false" class="block w-full py-4 border border-gray-200 text-center text-[10px] font-bold  tracking-[0.3em] hover:border-rose-300 hover:text-rose-500 transition-colors">
              Sign Out
            </button>
          </div>
        </div>
