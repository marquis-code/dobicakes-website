  ...
  <div class="section-container py-20 min-h-screen">
    <div class="max-w-4xl mx-auto space-y-12">
      <div class="text-center space-y-4">
        <span class="text-brand-gold text-[10px] font-bold  tracking-[0.4em]">My Account</span>
        <h1 class="text-4xl font-serif text-brand-charcoal ">Your Profile</h1>
      </div>

      <!-- Profile Card -->
      <div class="bg-white border border-gray-100 p-8 flex flex-col md:flex-row gap-8 items-center">
        <div class="w-20 h-20 bg-brand-gold rounded-full flex items-center justify-center text-white font-serif text-3xl shrink-0">
          {{ user?.firstName?.[0] || 'U' }}
        </div>
        <div class="flex-grow text-center md:text-left space-y-1">
          ...
          <p class="text-[10px] text-brand-muted  tracking-widest">{{ user?.email }}</p>
        </div>
        <button @click="logOut()" class="text-[10px] font-bold  tracking-widest text-rose-400 hover:text-rose-600 transition-colors border border-rose-200 px-6 py-3">
          Sign Out
        </button>
      </div>
      ...
      <div class="space-y-6">
        ...
        <div v-else-if="orders.length === 0" class="text-center py-16 border-2 border-dashed border-gray-100 space-y-4">
          <p class="text-brand-muted text-[10px]  tracking-widest">No orders yet.</p>
          <NuxtLink to="/shop" class="inline-block px-8 py-3 bg-brand-charcoal text-white text-[10px] font-bold  tracking-widest hover:bg-brand-gold transition-all">
            Start Shopping
          </NuxtLink>
        </div>
        ...
        <div v-else class="space-y-4">
          <div v-for="order in orders" :key="order._id" class="bg-white border border-gray-100 p-6 space-y-4 hover:shadow-md transition-all">
            <div class="flex flex-wrap justify-between items-start gap-4">
              <div class="space-y-1">
                <p class="text-[10px] text-brand-gold  tracking-widest font-bold">#{{ order._id.slice(-8) }}</p>
              ...
              <div class="flex items-center gap-4">
                <span class="text-sm font-serif text-brand-gold italic font-bold">₦{{ order.totalAmount.toLocaleString() }}</span>
