<template>
 ...
 <span class="text-sm font-bold text-brand-gold">{{ product.category }}</span>
 ...
 <h1 class="text-5xl md:text-7xl tracking-tight text-brand-charcoal">{{ product.name }}</h1>
 <div class="flex items-end gap-4">
 <p class="text-3xl text-brand-gold italic">₦{{ totalPrice.toLocaleString() }}</p>
 <p v-if="selectedSize?.priceOffset > 0" class="text-sm text-brand-muted tracking-widest pb-2">(Inc. ₦{{ selectedSize.priceOffset.toLocaleString() }} size offset)</p>
 </div>
 </div>
 ...
 <span class="text-sm font-bold tracking-[0.3em] text-brand-charcoal">Select Your Flavor</span>
 <div class="flex flex-wrap gap-3">
 <button v-for="flavor in product.flavors" :key="flavor"
 @click="selection.flavor = flavor"
 class="px-6 py-3 text-sm tracking-widest transition-all duration-500 border"
 ...
 <span class="text-sm font-bold tracking-[0.3em] text-brand-charcoal">Choose Size</span>
 ...
 <p class="text-sm font-bold tracking-widest" :class="selection.size === size.name ? 'text-brand-gold' : 'text-brand-charcoal'">{{ size.name }}</p>
 ...
 <!-- Custom Message -->
 <div class="space-y-4">
 <span class="text-sm font-bold tracking-[0.3em] text-brand-charcoal">Cake Inscription (Optional)</span>
 ...
 <button @click="handleAddToCart" class="flex-grow py-3 bg-brand-charcoal text-white text-sm font-bold transition-all duration-500 hover:bg-brand-gold shadow-xl active:scale-95">
 ...
 <li class="flex items-center gap-3 text-sm tracking-widest text-gray-800">
