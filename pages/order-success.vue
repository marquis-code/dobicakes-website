<template>
 ...
 <p class="text-sm tracking-[0.3em] text-gray-800">Verifying your payment...</p>
 </div>
 ...
 <div class="space-y-3">
 <h1 class="text-4xl text-brand-charcoal tracking-tight">Magnificent!</h1>
 <p class="text-sm text-brand-gold font-bold">Your Order is Confirmed</p>
 </div>
 ...
 <p class="text-sm tracking-[0.3em] text-brand-gold font-bold italic">Bank Transfer Required</p>
 ...
 <div v-for="(val, label) in { 'Bank': order.virtualAccount?.bankName, 'Account Number': order.virtualAccount?.accountNumber, 'Account Name': order.virtualAccount?.accountName }" :key="label" class="flex justify-between items-center text-sm tracking-widest">
 <span class="text-gray-800">{{ label }}</span>
 <span class="text-white font-bold">{{ val }}</span>
 </div>
 </div>
 <p v-else class="text-[11px] text-gray-800 tracking-widest leading-relaxed">
 ...
 <span class="text-gray-800 text-sm tracking-widest">Amount Due</span>
 <span class="text-2xl text-brand-gold">₦{{ order.totalAmount.toLocaleString() }}</span>
 </div>
 ...
 <p class="text-sm text-emerald-600 tracking-widest">Your payment has been confirmed and your order is being processed.</p>
 </div>
 </div>
 </div>
 ...
 <div class="space-y-1">
 <h2 class="text-2xl text-brand-charcoal ">Receipt</h2>
 ...
 <div>
 <p class="text-sm font-bold text-brand-charcoal tracking-widest">{{ item.name }}</p>
 <p class="text-[8px] text-brand-muted tracking-widest">Qty: {{ item.quantity }}</p>
 </div>
 </div>
 <p class="text-xs italic text-brand-gold">₦{{ (item.price * item.quantity).toLocaleString() }}</p>
 </div>
 </div>

 <div class="mt-12 space-y-3 pt-8 border-t border-dashed border-gray-200">
 <div class="flex justify-between text-sm tracking-widest text-brand-muted">
 <span>Subtotal</span>
 ...
 <div class="flex justify-between text-lg text-brand-charcoal pt-4">
 ...
 <p class="text-[8px] text-brand-muted font-bold">Thank You For Choosing Elegance</p>
 </div>
 </div>
 ...
 class="flex-grow group relative bg-white border border-brand-charcoal py-3 px-8 overflow-hidden transition-all duration-500 hover:text-white disabled:opacity-50"
 >
 ...
 <span class="text-sm font-bold tracking-[0.3em]">{{ downloading ? 'Preparing PDF...' : 'Download Receipt' }}</span>
 </div>
 </button>
 <NuxtLink to="/shop" class="flex-grow btn-primary text-center py-3 px-8 text-sm font-bold tracking-[0.3em]">
 Continue Shopping
 </NuxtLink>
 </div>
 </div>
 ...
 <div class="space-y-4">
 <h1 class="text-3xl text-red-500">Verification Failed</h1>
